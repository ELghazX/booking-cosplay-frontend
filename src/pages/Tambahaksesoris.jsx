...
export default function Tambahaksesoris() {
  ...
  e.preventDefault();
  if (!form.nama || !form.harga || !selectedTipe || !form.gambar) {
  ...
  const hargaInt = parseInt(form.harga, 10);
    ...
    try {
      ...
      const res = await api.post("/items/create-accessory", data);
      ...
      if (res.data.status) {
        ...
        Swal.fire("Gagal!", res.data.message || "Gagal menambah aksesoris", "error");
      }
    } catch (err) {
      Swal.fire("Gagal!", err.message || "Gagal menambah aksesoris", "error");
    }
  };
